...
- In the Northwind database, what is the type of relationship between the `Employee` and `Territory` tables?
  - The relationship between `Employee` and `Territory` tables is **one to many**. 
  - As 1 employee is associated with many Territories.
  
- What is a situation where a document store (like MongoDB) is appropriate, and what is a situation where it is not appropriate?
  - Document store like MongoDB is appropriate when the data is document oriented,requires high performance, high availability,high scalability, no rigid structure/schema required,can be distributed and when categorized as BigData.This can also be used for real time web applications. **A good usecase could be a catalog and blogs**.
  - This would not be the best fit where relationships bare present between the data as in a relational database. MongoDB is also not ACID compliant. **A bad use case would be Banks.**
- What is "NewSQL", and what is it trying to achieve?
  - A NewSQL database tries to take the advantages of both SQL and NoSQL and combine them together. This would guarantee ACID properties and still create a scalable database.
